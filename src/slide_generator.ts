...
import { Stylesheet } from './parser/css';
...
export default class SlideGenerator {
    ...
    //     let api = SlidesV1.slides({auth: oauth2Client})
    ...
    public static async create(api: SlidesV1.Slides, presentation: SlidesV1.Schema$Presentation): Promise<SlideGenerator> {
        return new SlideGenerator(api, presentation);
    }


    public async generateFromMarkdown(markdown, { css }): Promise<string> {
    ...
    protected populateSlides(): SlidesV1.Schema$BatchUpdatePresentationRequest {
        ...
        console.log("polulateSlides", this.slides)
        for (let slide of this.slides) {
            const layout = matchLayout(this.presentation, slide);
            layout.appendContentRequests(batch.requests);
        }
        console.log("populateSlides", batch)
        return batch;
    }
    //
    // /**
    ...
    protected async updatePresentation(batch): Promise<void> {
        console.log('Updating presentation: %O', batch);
